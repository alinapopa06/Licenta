...
def draw_map():
    ...
    while True:
        for event in pygame.event.get():
            ...
            elif event.type == pygame.MOUSEBUTTONDOWN and draw_decision_enabled and event.button == MIDDLE and draw_decision_enabled:
            ...
            elif event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT and draw_decision_enabled:
                print("Click: ({})".format(event.pos))
                pygame.draw.circle(window, WHITE, event.pos, 5, 0)
                draw_text('+', BLACK, 20, event.pos[0], event.pos[1])
            elif event.type == pygame.MOUSEBUTTONDOWN and event.button == RIGHT and draw_decision_enabled:
                print("Click: ({})".format(event.pos))
                pygame.draw.circle(window, WHITE, event.pos, 5, 0)
                draw_text('-', BLACK, 20, event.pos[0], event.pos[1])
